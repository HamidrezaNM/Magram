...
export default (conference, isAnswer = false, isPresentation = false, isP2p = false) => {
  ...
  const addTransport = () => {
    ...
    add(`a=ice-pwd:${pwd}`);
    add('a=ice-options:trickle')           // ! test
    fingerprints.forEach((fingerprint) => {
      add(`a=fingerprint:${fingerprint.hash} ${fingerprint.fingerprint}`);
      add(`a=setup:${true ? (fingerprint.setup) : 'passive'}`); // isP2p
    });

    candidates.forEach(addCandidate);
  };

  const addPayloadType = (type) => {
    // const {
  ...
  const addSsrcEntry = (entry) => {
    ...
    add(`m=${type} ${entry.isMain ? 1 : 0} UDP/TLS/RTP/SAVPF ${payloadTypes.map((l) => l.id).join(' ')}`);
