...
export function getCountryCodesByIso(phoneCodeList, iso) {
    return phoneCodeList.filter((country) => country.iso2 === iso);
}

export function getCountryFromPhoneNumber(phoneCodeList, input = '') {
    ...
    const codesWithPrefix = possibleCountries
...
export function formatPhoneNumber(input, country) {
    if (!input) {
        return '';
    }
...
export function formatPhoneNumberWithCode(phoneCodeList, phoneNumber) {
    if (!phoneNumber) {
        return '';
    }
