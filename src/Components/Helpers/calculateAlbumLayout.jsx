...
import { calculateMediaDimensions, getAvailableWidth } from "../App/Message/MessageMedia";
import { clamp } from "../Util/math";
import { getMediaDimensions } from "./messages";
...
function getRatios(messages) {
    const isOutgoing = messages[0].out;
...
export function calculateAlbumLayout(
    ...
    const spacing = 2;
    const ratios = getRatios(album);
    const proportions = getProportions(ratios);
    const averageRatio = getAverageRatio(ratios);
    const albumCount = ratios.length;
    const forceCalc = ratios.some((ratio) => ratio > 2);
    const maxWidth = getAvailableWidth(isOwn, false, noAvatars);
