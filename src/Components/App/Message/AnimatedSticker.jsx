...
const AnimatedSticker = forwardRef(({ media, size, _width, _height, isCustomEmoji = false, returnData = false, autoPlay = true, loop = true, setProgress, isLoaded, setIsLoaded, uploading, setIsDownloading }, ref) => {
    ...
    return <Transition state={true}>
        ...
        {isWebp ? <img width={20} height={20} src={data} /> :
            isWebm ? <video width={20} height={20} src={data} autoPlay={autoPlay} loop /> :
                data && <LottiePlayer returnData={returnData} data={data} fileId={media.document.id.value} autoPlay={autoPlay} loop={loop} width={width} height={height} />
        }
    </Transition>
})
